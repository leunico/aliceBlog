...
{
    //定义sql语句
    const _SQL_SELECT_PAGE_LIST = 'SELECT %s from %s WHERE %s ORDER BY %s LIMIT %s;';
    //查询分页信息
    const _SQL_SELECT_LIST = 'SELECT %s from %s WHERE %s ORDER BY %s';
    //查询信息
    const _SQL_SELECT_ONE = 'SELECT %s from %s WHERE %s LIMIT 0,1;';
    //查询信息
    const _SQL_COUNT = 'SELECT COUNT(*) AS num FROM %s WHERE %s;';
    //查询合条件的记录数
    const _SQL_UPDATE = 'UPDATE %s SET %s WHERE %s;';
    //更新SQL
    const _SQL_DELETE = 'DELETE FROM %s WHERE %s;';
    //删除信息
    const _SQL_INSERT = 'INSERT INTO %s (%s) VALUES (%s);';
    //插入单条记录SQL
