...
namespace AliceFrameWork\weixin;
...
class WeixinClass{
    ...
    private function receiveText($object){
        ...
        if (strstr($keyword, "您好") || strstr($keyword, "你好") || strstr($keyword, "在吗")){
            ...
            }else if(strstr($keyword, "爱")){
                $content = "敬往事一杯酒，再爱不回头...";
            }else if($keyword =="你喜欢谁"){
                $content = "哈哈哈哈哈哈哈";
            }else{
                $content = "你发送的内容是：".$object->Content."\n时间:".date("Y-m-d H:i:s",time())."\nAlice的窝回答不了，么么哒！";//"\n".$object->FromUserName.
    ...
    private function receiveEvent($object){
        ...
        switch ($object->Event){
            case "subscribe":                            
                ...
                if (isset($return)){
                    $contentStr = is_array($return) ? "你已经关注且注册过帐号\n请再扫描二维码登录......\n帐号是:".$return[0]["username"] : "感谢关注我的微信公众平台！\n请再扫描二维码登录......\n帐号:".$info["nickname"]."\n密码:".$password;
                }else{
            ...
            case "SCAN": 
                ...
                if(!empty($return)){
                    ...
                    $contentStr  = empty($login) ? "由于数据处理错误，登录失败！" : $info["nickname"]."在AliceBlog的帐号已经登录啦，请等待跳转~\n登录数字:".$object->EventKey;
                }else{
            ...
            case "CLICK":
                switch ($object->EventKey){
                    case "company":
                        $contentStr[] = array("Title" =>"个人简介", 
                        "Description" =>"我的个人微博及个人Blog", 
                        "PicUrl" =>"http://discuz.comli.com/weixin/weather/icon/cartoon.jpg", 
                        "Url" =>"http://520.leunico.sinaapp.com/");
                        break;
                    case "游戏":
                        $contentStr[] = array("Title" =>"OAuth2.0网页授权演示", 
                        "Description" =>"OAuth2.0网页授权演示", 
                        "PicUrl" =>"http://discuz.comli.com/weixin/weather/icon/cartoon.jpg", 
                        "Url" =>"https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxb33bef172a0860cf&redirect_uri=http://520.leunico.sinaapp.com/wechat/oauth.php&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect");
                        break;
                    default:
                        $contentStr[] = array("Title" =>"默认菜单回复", 
                        "Description" =>"您正在使用的是Leunico测试帐号的自定义菜单测试接口", 
                        "PicUrl" =>"http://discuz.comli.com/weixin/weather/icon/cartoon.jpg", 
                        "Url" =>"http://520.leunico.sinaapp.com/");
                        break;
                }
