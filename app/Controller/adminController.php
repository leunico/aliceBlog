...
class adminController extends Controller
{
    ...
    public static function is_admin($id = '')
    {
        ...
        if ($id !== '') {
            ...
            if ($article['uid'] !== $loginInfo['id'] && $result['is_admin'] == '0') {
                View::AdminErrorMessage('goback', '你没有这篇文章的操作权限！');
            }
            return $result['is_admin'];
        } else {
            if ($result['is_admin'] == '0') {
                View::AdminErrorMessage('goback', '对不起，你没有这个操作的权限！');
            }
        }
    }
    ...
    public function login()
    {
        ...
        $ret = array();
        //生成微信登录用的二维码
        $weixinObj = self::$models->Weixin;
        $ret['scene_id'] = rand(100, 999);
        //不安全，小博客就不考虑这么多了
    ...
    public function mem_updata()
    {
        self::is_admin();
        $admin = self::$models->Admin;
        $admin->MemUpdata(self::$models->Memcache);
        die('sos');
    }
