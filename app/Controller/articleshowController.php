...
class articleshowController{
	...
	public function index($id=''){
        ...
        if($id == '') View::AdminErrorMessage('goback', '木有选择文章哦！');       
        ...
        if(empty($ret)){            
            $ret = array();            
            $ret['articleShow'] = ArticleModel::getArticleShow($id);
            if(empty($ret['articleShow'])) NotFound();
            $ret['articleShow']['tag'] = explode('|', $ret['articleShow']['tag']);        
            $ret['articleRelevant'] = ArticleModel::getArticleRelevant($ret['articleShow']['mid'],$ret['articleShow']['id']);            
        ...
        $ret['comments'] = CommentModel::getArticleCommentList($id,$page);
        if(!empty($ret['comments']['counts'])) $ret['articleShow']['counts'] = array_pop($ret['comments']);
        if(isset($ret['comments']['page'])) $ret['articleShow']['commentPage_nav'] = array_pop($ret['comments']);
        ArticleModel::updatePlus($id,'clicks');  //文章的点击数+1
    ...
    public function addcomment(){
        ...
        if ( 'POST' != $_SERVER['REQUEST_METHOD'] ){ //这里做一个csrf攻击的防范，当然还可以加Referer的验证，如果要最安全还是得用token令牌      
             header('Allow: POST');               
        ...
        if( 6 > strlen($fields['email']) || '' == $fields['nickname']) AjaxError( '请填写昵称和邮箱！');         
        if( !Is_email($fields['email'])) AjaxError( '请填写有效的邮箱地址！');        
        if( '' == $fields['contents'] ) AjaxError( '请写点评论！'); 
        //CommentModel::IpLimit($fields['ip']); //防止评论灌水攻击        
        if(strstr($fields['cid'],'-')){           
            $parents = explode('-', $fields['cid']);            
            $fields['cid'] = $parents[0];
            $tomail = $parents[1];
            $commentp = CommentModel::getOneComment('id',$tomail);                                 
            ...
            $commentp = CommentModel::getOneComment('id',$fields['cid']);
            $fields['parent'] = $commentp ? $commentp['id'].','.$commentp['nickname'] : '';
            $fidname = "<a href=\"#comment-".$commentp['id']."\" rel=\"nofollow\" class=\"cute\">@".$commentp['nickname']."</a>";
        }else{
            $fields['parent'] = '';
        ...
        $result = CommentModel::InsertComment($fields);
        if(!$result){
            AjaxError('评论添加失败，多次失败请联系站长！');
        }else{
            CommentModel::Ifuser($fields['nickname'],$fields['email'],$fields['website']); //记录游客信息
            if(EMAIL_SENT_FOR_REPLY && $fields['cid'] > 0 && !empty($commentp)) CommentModel::SendMail($tomail,$fields['contents'],$commentp); //邮件  
    ...
    public function scoreajax(){
        ...
        $data = array();      
        $addScore = Request::getCookie('add_score_'.$fields['um_id']); //判断是否24小时内已经投过了。cookie判断，伪验证!安全点就使用ip验证。
        ...
        if($fields['um_action'] == 'ding'){          
            $result = ArticleModel::updatePlus($fields['um_id'],'good_num');
        }elseif($fields['um_action'] == 'xu'){      
            $result = ArticleModel::updatePlus($fields['um_id'],'bad_num');          
        }
